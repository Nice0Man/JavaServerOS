...
import server.endpoints.Authenticator;
import server.http.request.Handler;
import server.http.request.Listener;
...
public class App implements Serializable {
        ...
        try {
            ...
            while (true) {
                ...
                if (Config.username != null && Config.password != null) {
                    try {
                        boolean[] authResult = authStatus(cloneSocket(socket));
    ...
    public static boolean[] authStatus(Socket socket) throws IOException {
        ...
        while (line != null) {
            if (line.contains("Authorization: Basic")) {
                auth[0] = true;
                auth[1] = Authenticator.authenticate(socket, line.split("\\s+")[2]);
                return auth;
            }
            line = in.readLine();
        }
    ...
    public static Socket cloneSocket(Socket socket) throws IOException, ClassNotFoundException {
        ...
        return (Socket) new ObjectInputStream(byteArrayInputStream).readObject();
    }
